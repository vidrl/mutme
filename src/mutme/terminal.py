...
def run(
    ...
    allow_x_wildcards: bool = typer.Option(
        False,
        "--allow-x-wildcards",
        help=(
            "Allow 'X' in annotation-table amino-acid substitutions or insertions to act "
            "as a wildcard matching any single amino acid in detected mutations. "
            "Applies to substitutions (e.g. S:N87X) and insertions (e.g. S:214:NXY) only."
        ),
    ),
    x_charset: str = typer.Option(
        AMINO_ACIDS,
        "--x-charset",
        metavar="CHARS",
        show_default=True,
        help=(
            "Allowed characters that 'X' can match when --allow-x-wildcards is enabled. "
    ...
    nextclade_preset: AlignmentPreset = typer.Option(
    ...
    if precomputed_tsv:
        ...
        try:
            nextclade_output, _ = run_nextclade(
                ...
                alignment_preset=nextclade_preset,
